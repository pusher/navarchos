...
// failedPodError implements the Error interface and provides a method read the
// aggregate error out as a map podName[error]
...
func (h *NodeReplacementHandler) handleInProgress(instance *navarchosv1alpha1.NodeReplacement) (*status.Result, error) {
	// evictedPods captures all pod names that are succesfully evicted
...
// package. If the provided string contains more than one set of double quotes
func parsePodName(err string) (string, error) {
	...
	if len(split) != 3 {
		return "", fmt.Errorf("failed to parse error %q: does not contain exactly one double quoted substring", err)
	}
	return split[1], nil
}
